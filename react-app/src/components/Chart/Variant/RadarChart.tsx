...
import { HvAPlotProps, Layer } from "../../Types/types";
...
const cleanData = (
  allAxis: string[],
  data: HvAPlotProps["data"],
  maxValues: { [axis: string]: number }
) => {
  return data.map((item, radialChartSeriesIndex) =>
    allAxis.reduce((series, axis, radialChartAxisIndex) => {
      ...
      if (
        !isNaN(numericValue) &&
        (maxValues[axis] === undefined || numericValue > maxValues[axis])
      ) {
...
const wrap = (
  text: Selection<SVGTextElement, any, any, any>,
  width: number
) => {
  text.each(function () {
    ...
    if (chartRef.current && data) {
      ...
      const g = svg
        ...
        .attr(
          "transform",
          "translate(" +
            (config.width / 2 + config.margin.left + config.labelOffset / 2) +
            "," +
            (config.height / 2 + config.margin.top + config.labelOffset / 2) +
            ")"
        );
      ...
      axisGrid
        ...
        .attr("r", (d, _i) => (radius / config.levels) * d)
        .style("fill", "#CDCDCD")
        .style("stroke", "#CDCDCD")
      ...
      axisGrid
        ...
        .attr("fill", "#737373")
        .text((d, _i) => Math.round((maxValue * d) / config.levels));
      ...
      blobWrapper
        ...
        .attr("d", (d, _i) => d && radarLine(d))
        .style("fill", (_d, i) => config.color(i.toString()))
      ...
      blobWrapper
        ...
        .style("stroke", (_d, i) => config.color(i.toString()))
        .style("fill", "none")
        .style("filter", "url(#glow)");
      ...
      blobWrapper
        ...
        .attr(
          ...
          (d) =>
            rScale(
              Math.round((config.maxValue * d.value) / maxValues[d.axis])
      ...
      blobCircleWrapper
        ...
        .on("mouseover", function (_d: RadarPoint, data) {
